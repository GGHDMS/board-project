...
class ArticleWithCommentsResponseTest {
    ...
    @DisplayName("게시글 + 자식 댓글이 없는 댓글 dto를 api 응답으로 변환할 때, 댓글을 시간 내림차순 + ID 오름차순으로 정리한다.")
    @Test
    public void convertPostAndCommentWithoutRepliesToResponse() throws Exception {
        //given
        LocalDateTime now = LocalDateTime.now();
        Set<ArticleCommentDto> articleCommentDtos = Set.of( // 정렬할 commentDto
    ...
    @Test
    public void convertPostAndCommentWithSortCommentsByHierarchy() throws Exception {
        //given
        LocalDateTime now = LocalDateTime.now();
        Set<ArticleCommentDto> articleCommentDtos = Set.of( // 정렬할 commentDto
                createArticleCommentDto(1L, null, now),
        ...
        //then
        assertThat(actual.getArticleCommentsResponse())
                .containsExactly( //처음에는 부모 댓글만 나와야 된다.
                        createArticleCommentResponse(5L, null, now.plusDays(5L)),
                        createArticleCommentResponse(6L, null, now.plusDays(4L)),
                        createArticleCommentResponse(1L, null, now)
                )
                .flatExtracting(ArticleCommentResponse::getChildComments) // 자식 comments 를 추출 그 후 flat 하게 펼친다.
                .containsExactly(
    ...
    @Test
    public void convertPostsAndCommentsWithUnlimitedDepth() throws Exception {
        //given
        LocalDateTime now = LocalDateTime.now();
        Set<ArticleCommentDto> articleCommentDtos = Set.of( // 정렬할 commentDto
                createArticleCommentDto(1L, null, now),
