...
class HashtagServiceTest {
    ...
    @MethodSource
    @ParameterizedTest(name = "[{index}] \"{0}\" => {1}") // index 는 테스트 순서, 0은 input, 1은 expected
    public void parseUniqueHashtags(String content, Set<String> expected) throws Exception{
        //given
        ...
        //then
        assertThat(actual).containsExactlyInAnyOrderElementsOf(expected); // 정확하게, 순서는 상관없이
        then(hashtagRepository).shouldHaveNoInteractions();
    }


    static Stream<Arguments> parseUniqueHashtags() {
        return Stream.of(  // arguments(입력값, 기대값)
    ...
    @DisplayName("헤시태그 이름들을 입력하면, 저장된 해시태그 중 이름에 매칭하는 것들을 중복 없이 반환한다.")
    @Test
    public void getUniqueMatchingHashtags() throws Exception{
        //given
